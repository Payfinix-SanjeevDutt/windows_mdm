...
def discovery_service():
    ...
    try:
        ...
        message_id_match = re.search(
            r'<a:MessageID>(.*?)<\/a:MessageID>', body)
        ...
        if auth_policy == "Federated":
            extra_params = f"<AuthenticationServiceUrl>https://{
                domain}/EnrollmentServer/Auth</AuthenticationServiceUrl>"
            ...
            response_payload = f"""<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"
                        ...
                        <s:Body>
                            <DiscoverResponse xmlns="http://schemas.microsoft.com/windows/management/2012/01/enrollment">
                                <DiscoverResult>
                                    <w:AuthPolicy>{auth_policy}</w:AuthPolicy>
                                    <w:EnrollmentVersion>4.0</w:EnrollmentVersion>
                                    <w:EnrollmentPolicyServiceUrl>https://{domain}/EnrollmentServer/Policies.svc</w:EnrollmentPolicyServiceUrl>
                                    <w:EnrollmentServiceUrl>https://{domain}/EnrollmentServer/Enrollment.svc</w:EnrollmentServiceUrl>
                                    <w:TermsOfUseUrl>https://windowsmdm.sujanix.com/EnrollmentServer/TermsofUse</w:TermsOfUseUrl>
                                    {extra_params}
                                </DiscoverResult> 
                            </DiscoverResponse>
                        </s:Body>
...
@app.route('/EnrollmentServer/Discoverys.svc', methods=['GET', 'POST'])
            ...
            <s:Body>
                <DiscoverResponse xmlns="http://schemas.microsoft.com/windows/management/2012/01/enrollment">
                    <DiscoverResult>
                        ...
                        <AuthenticationServiceUrl>https://{domain}/EnrollmentServer/Auth</AuthenticationServiceUrl>
                    </DiscoverResult>
                </DiscoverResponse>
            </s:Body>
        </s:Envelope>"""
...
# Abhis's code
@app.route('/EnrollmentServer/Auth', methods=['GET', 'POST'])
def auth_handler():
    import requests
    ...
    redirect_uri = "https://windowsmdm.sujanix.com/EnrollmentServer/Auth"
    authority = f"https://login.microsoftonline.com/{tenant_id}"
    token_endpoint = f"{authority}/oauth2/v2.0/token"

    # If 'code' is in the query, it means the user authenticated and Azure AD returned an authorization code
    code = request.args.get("code")
    appru = request.args.get("appru", "#")
    ...
    if code:
        ...
        if response.status_code == 200:
            ...
            return f"""
            <h3>MDM Federated Login Successful</h3>
            <form method="post" action="{appru}">
            ...
            return f"Error fetching token: {response.json()}"
...
def enroll_service():
    try:
        ...
        # Extract the token (assuming it's passed in the <wresult> tag)
        ...
        jwks_uri = f"https://login.microsoftonline.com/{
