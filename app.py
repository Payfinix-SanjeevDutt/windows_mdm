...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'edad563bccd0896fdf438d700cf6a5f5a6d0f3334056237263c54e57280941be')

def apply_windows_update_policy(settings):
    try:
        key_path = r"SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update"
        key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, key_path, 0, winreg.KEY_SET_VALUE)
        
        if 'AutoUpdateEnabled' in settings:
            winreg.SetValueEx(key, 'AUOptions', 0, winreg.REG_DWORD, 
                              4 if settings['AutoUpdateEnabled'] else 1)
        
        if 'ScheduledInstallDay' in settings:
            winreg.SetValueEx(key, 'ScheduledInstallDay', 0, winreg.REG_DWORD, settings['ScheduledInstallDay'])
        
        if 'ScheduledInstallTime' in settings:
            winreg.SetValueEx(key, 'ScheduledInstallTime', 0, winreg.REG_DWORD, settings['ScheduledInstallTime'])
        
        winreg.CloseKey(key)
        subprocess.run(['net', 'stop', 'wuauserv'], check=True)
        subprocess.run(['net', 'start', 'wuauserv'], check=True)
        
        return True, "Windows Update policy applied successfully"
    except Exception as e:
        return False, f"Error applying Windows Update policy: {str(e)}"

@app.route('/.well-known/enterpriseregistration/<domain>/v2/discovery', methods=['GET'])
@app.route('/EnrollmentServer/Discovery.svc/Windows/Discover', methods=['POST'])
@app.route('/EnrollmentServer/Discovery.svc', methods=['POST', 'GET'])
def autodiscover(domain=None):
...
<DiscoveryResponse xmlns="http://schemas.microsoft.com/windows/management/2012/01/enrollment">
    ...
    <DeviceManagementServiceUrl>{server_url}/EnrollmentServer/Discovery.svc</DeviceManagementServiceUrl>
</DiscoveryResponse>"""
    return Response(discovery_xml, mimetype='application/xml')

@app.route('/EnrollmentServer/Auth.svc', methods=['POST'])
def auth_service():
    auth_response = """<?xml version="1.0" encoding="utf-8"?>
<AuthResponse xmlns="http://schemas.microsoft.com/windows/management/2012/01/enrollment">
    <AuthToken>dummy_token</AuthToken>
    <AuthPolicy>OnPremise</AuthPolicy>
    <Success>true</Success>
</AuthResponse>"""
    return Response(auth_response, mimetype='application/xml')

@app.route('/EnrollmentServer/Enrollment.svc', methods=['POST'])
def enrollment():
    device_id = str(uuid.uuid4())
    enrollment_xml = f"""<?xml version="1.0" encoding="utf-8"?>
<wap-provisioningdoc xmlns="http://www.w3.org/2001/XMLSchema-instance">
    <characteristic type="MDMClient">
        <parm name="EntDMID" value="{device_id}"/>
        <parm name="DeviceID" value="{device_id}"/>
    </characteristic>
</wap-provisioningdoc>"""
    return Response(enrollment_xml, mimetype='application/xml')

@app.route('/MDM/Manage.svc', methods=['POST'])
