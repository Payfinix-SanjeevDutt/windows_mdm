...
def home():
    return jsonify({
        ...
        "endpoints": {
            "discovery": f"https://{domain}/EnrollmentServer/Discovery.svc",
            "terms_of_use": f"https://{domain}/EnrollmentServer/TermsofUse",
            "compliance": f"https://{domain}/EnrollmentServer/Compliance.svc",
            "enrollment": f"https://{domain}/EnrollmentServer/Enrollment.svc"
        },
    })
...
def discovery_service():
    ...
    if request.method == 'GET':
        ...
        return Response(f"https://{domain}/EnrollmentServer/Enrollment.svc", status=200)
    ...
    try:
        ...
        # Added CertificatePolicy element to instruct the device on certificate issuance.
        certificate_policy = f"""
            <CertificatePolicy>
                <CertificateTemplate>MDMDeviceTemplate</CertificateTemplate>
...
def authentication_service():
    if request.method == 'GET':
        ...
        redirect_uri = f"https://{domain}/auth/redirect"
        ...
        # For POST requests, additional handling can be added if needed.
...
def generate_provisioning_xml(cert_str):
    ...
    provisioning_xml = f"""<?xml version="1.0" encoding="utf-8"?>
                        ...
                        <characteristic type="APPLICATION">
                            ...
                            <parm name="ADDR" value="https://{domain}/EnrollmentServer/Enrollment.svc"/>
                        </characteristic>
                        </wap-provisioningdoc>"""
...
def enroll_service():
    ...
    try:
        ...
        body = request.get_data(as_text=True)
        print("HTTP Enrollment Request Body:", body)
        ...
        # 3. Parse SOAP XML envelope
        envelope = ET.fromstring(body)

        # 4. Retrieve <wsse:BinarySecurityToken> element
        ...
        # 9. Load the server's Root CA certificate and private key.
        try:
            with open("./identities/root.crt", "rb") as cert_file:
                root_certificate_pem = cert_file.read()
            with open("./identities/root.key", "rb") as key_file:
        ...
        # 10. Parse the CA certificate and key, and convert to DER.
        try:
        ...
        # 11. Decode the BinarySecurityToken (Base64)
        try:
        ...
        # 12. Enforce that the token is a PKCS#10 CSR in PEM format.
        if decoded_token.strip().startswith(b"-----BEGIN CERTIFICATE REQUEST-----"):
            try:
                csr = x509.load_pem_x509_csr(decoded_token, default_backend())
                print("CSR loaded successfully (PEM) from request.")
            except Exception as e:
                print("Error loading CSR:", e)
                return Response("Invalid CSR format", status=400)
        else:
            # If the token is not a valid CSR, return an error.
            print("Expected a PKCS#10 CSR in PEM format, but received a token that is not a CSR.")
            return Response("Enrollment requires a PKCS#10 CSR in PEM format", status=400)
        ...
        # 14. Generate the client certificate using the public key from the CSR.
        public_key = csr.public_key()
        subject_value = device_id  # Use device ID as the certificate subject
        ...
        certificate_builder = certificate_builder.subject_name(
            x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, subject_value)])
        )
        # Use the CA's subject as issuer.
        ...
        # Convert the CA key to a cryptography key.
        ...
        # 16. Determine certificate store based on EnrollmentType.
...
<wap-provisioningdoc version="1.1">
    ...
    <s:Body>
        ...
        print("SOAP Response Payload:", response_xml)
...
#         tenant_id = Config.TENANT_ID
#         client_id = Config.CLIENT_ID
#         redirect_uri = "https://windowsmdm.sujanix.com/auth/redirect"
#         authority = f"https://login.microsoftonline.com/{tenant_id}"
#         auth_url = f"{authority}/oauth2/v2.0/authorize"
...
#         # 12. Determine whether the token is a CSR or a JWT.
#         csr = None
#         upn = None
#         if decoded_token.strip().startswith(b"-----BEGIN CERTIFICATE REQUEST-----"):
#             # It is a CSR, load it normally.
#             csr = x509.load_pem_x509_csr(decoded_token, default_backend())
#             print("CSR loaded successfully (PEM) from request.")
#         else:
#             # Assume it's an identity token (e.g. JWT)
#             try:
#                 token_str = decoded_token.decode('utf-8')
#                 print("Decoded token as string:", token_str[:100])
#                 # Here we use an unverified decode; in production verify the signature!
#                 claims = jwt.get_unverified_claims(token_str)
#                 upn = claims.get("upn") or claims.get("preferred_username")
#                 if not upn:
#                     return Response("UPN not found in token", status=400)
#                 print("Extracted UPN from token:", upn)
#             except Exception as e:
#                 print("Error decoding JWT token:", e)
#                 return Response("Invalid identity token", status=400)

#         # 13. Verify the CSR signature if a CSR was provided.
#         if csr is not None:
#             try:
#                 csr.public_key().verify(
#                     csr.signature,
...
#         # 14. Generate the client certificate.
#         # If a CSR was provided, use its public key; otherwise generate a new key pair.
#         if csr is not None:
#             public_key = csr.public_key()
#             subject_value = device_id
#         else:
#             # For federated auth without CSR, generate a new key pair.
...
#         certificate_builder = certificate_builder.subject_name(
#             x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, subject_value)]))
#         # Use the CA's subject as the issuer.
#         issuer_components = root_cert_parsed.get_subject().get_components()
#         issuer_attributes = []
#         for k, v in issuer_components:
#             key_str = k.decode('utf-8')
